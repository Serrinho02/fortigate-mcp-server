...
class TestFortiGateAPIAsync:
    ...
    @pytest.mark.asyncio
    async def test_get_ipsec_phase1_list(self):
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"results": []}) as mock:
            await self.api.get_ipsec_phase1_list()
            mock.assert_called_once_with("GET", "cmdb/vpn.ipsec/phase1-interface", vdom=None)

    @pytest.mark.asyncio
    async def test_get_ipsec_phase1_detail(self):
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"results": {}}) as mock:
            await self.api.get_ipsec_phase1_detail("tunnel1")
            mock.assert_called_once_with("GET", "cmdb/vpn.ipsec/phase1-interface/tunnel1", vdom=None)

    @pytest.mark.asyncio
    async def test_create_ipsec_phase1(self):
        phase1_data = {"name": "tunnel1", "interface": "wan1", "remote-gw": "203.0.113.1"}
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"status": "success"}) as mock:
            await self.api.create_ipsec_phase1(phase1_data)
            mock.assert_called_once_with(
                "POST", "cmdb/vpn.ipsec/phase1-interface", data=phase1_data, vdom=None
            )

    @pytest.mark.asyncio
    async def test_update_ipsec_phase1(self):
        phase1_data = {"remote-gw": "203.0.113.2"}
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"status": "success"}) as mock:
            await self.api.update_ipsec_phase1("tunnel1", phase1_data)
            mock.assert_called_once_with(
                "PUT", "cmdb/vpn.ipsec/phase1-interface/tunnel1", data=phase1_data, vdom=None
            )

    @pytest.mark.asyncio
    async def test_delete_ipsec_phase1(self):
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"status": "success"}) as mock:
            await self.api.delete_ipsec_phase1("tunnel1")
            mock.assert_called_once_with("DELETE", "cmdb/vpn.ipsec/phase1-interface/tunnel1", vdom=None)

    # --- IPsec phase2 ---------------------------------------------------

    @pytest.mark.asyncio
    async def test_get_ipsec_phase2_list(self):
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"results": []}) as mock:
            await self.api.get_ipsec_phase2_list()
            mock.assert_called_once_with("GET", "cmdb/vpn.ipsec/phase2-interface", vdom=None)

    @pytest.mark.asyncio
    async def test_get_ipsec_phase2_detail(self):
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"results": {}}) as mock:
            await self.api.get_ipsec_phase2_detail("selector1")
            mock.assert_called_once_with("GET", "cmdb/vpn.ipsec/phase2-interface/selector1", vdom=None)

    @pytest.mark.asyncio
    async def test_create_ipsec_phase2(self):
        phase2_data = {"name": "selector1", "phase1name": "tunnel1"}
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"status": "success"}) as mock:
            await self.api.create_ipsec_phase2(phase2_data)
            mock.assert_called_once_with(
                "POST", "cmdb/vpn.ipsec/phase2-interface", data=phase2_data, vdom=None
            )

    @pytest.mark.asyncio
    async def test_update_ipsec_phase2(self):
        phase2_data = {"src-subnet": "10.0.0.0/24"}
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"status": "success"}) as mock:
            await self.api.update_ipsec_phase2("selector1", phase2_data)
            mock.assert_called_once_with(
                "PUT", "cmdb/vpn.ipsec/phase2-interface/selector1", data=phase2_data, vdom=None
            )

    @pytest.mark.asyncio
    async def test_delete_ipsec_phase2(self):
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"status": "success"}) as mock:
            await self.api.delete_ipsec_phase2("selector1")
            mock.assert_called_once_with("DELETE", "cmdb/vpn.ipsec/phase2-interface/selector1", vdom=None)

    # --- IPsec status + SSL VPN ---------------------------------------------------

    @pytest.mark.asyncio
    async def test_get_ipsec_tunnel_status(self):
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"results": []}) as mock:
            await self.api.get_ipsec_tunnel_status()
            mock.assert_called_once_with("GET", "monitor/vpn/ipsec", vdom=None)

    @pytest.mark.asyncio
    async def test_get_ssl_vpn_settings(self):
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"results": {}}) as mock:
            await self.api.get_ssl_vpn_settings()
            mock.assert_called_once_with("GET", "cmdb/vpn.ssl/settings", vdom=None)

    @pytest.mark.asyncio
    async def test_update_ssl_vpn_settings(self):
        settings_data = {"port": 10443}
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"status": "success"}) as mock:
            await self.api.update_ssl_vpn_settings(settings_data)
            mock.assert_called_once_with("PUT", "cmdb/vpn.ssl/settings", data=settings_data, vdom=None)

    @pytest.mark.asyncio
    async def test_get_ssl_vpn_sessions(self):
        with patch.object(self.api, '_make_request', new_callable=AsyncMock, return_value={"results": []}) as mock:
            await self.api.get_ssl_vpn_sessions()
            mock.assert_called_once_with("GET", "monitor/vpn/ssl", vdom=None)
